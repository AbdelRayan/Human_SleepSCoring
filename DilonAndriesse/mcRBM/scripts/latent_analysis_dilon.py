...
class StatesAnalysis(object):   
    ...
    def __init__(self, refDir, expConfigFilename, epochID, threshold, multi, norm, features, groupNames):
        ...
        self.norm = norm
        # which features were used for the analysis : EEG bands,
        # EEG ratios, pca
    ...
    def analyzeStates(self):
        ...
        for i in self.uniqueStates[:, 0]:
            ...
            dPlotEEG = [self.dinit[idx, j] for j in range(self.dinit.shape[1])]  
            # print("dPlotEEG :", dPlotEEG)  
            # print("len(idx) :", len(idx))
            # print("i :", i)
            # print("EEG_labels", EEG_labels)
    ...
    def groupStatistics(self):
        ...
        for lstate in self.lstatesIDs:
            ...
            length_n2 = round((len(np.where((latent_frames[:, latent_frames.shape[1]-3]==2))[0])/float(len(latent_frames))),3)
            length_n3 = round((len(np.where((latent_frames[:, latent_frames.shape[1]-3]==3))[0])/float(len(latent_frames))),3)
            ...
            fig.suptitle('Latent State ' + str(lstate) + 
                         ...
                         '%, N1 Sleep: ' + str(round(length_n1,3)*100) + 
                         '%, N2 Sleep: ' + str(round(length_n2,3)*100) +
            ...
            fig.suptitle('LS ' + str(lstate) + 
                         ' - Total: ' + str(len(latent_frames)) + 
                         ' epochs' + '\nWAKE: ' + str(length_awake*100) + 
                         '%, N1: ' + str(length_n1*100) + 
                         '%, N2: ' + str(length_n2*100) + 
                         '%, N3: ' + str(length_n3*100) + 
                         '%, REM: ' + str(length_rem*100) + 
    ...
    def visibleDistributions(self):
        ...
        self.initFeatures = ['IndexW', 'IndexR', 'IndexN', 'Index1', 'Index2', 'Index3', 'Index4', '0-0.5hz', 'Theta', 'Delta', 'AperiodicExponent', 'DFA', 'MSE', 'EOG']
    ...
    def entropyMIcontrol(self):
        ...
        with open ('./entropyMI/mutualInformation.txt','a') as f:
            ...
            f.write("\n The MI/H(n2) = %s" %(MI_stage[2]/stagesH[2]))
            f.write("\n The MI/H(n3) = %s" %(MI_stage[3]/stagesH[3]))
            f.write("\n The MI/H(rem) = %s\n" %(MI_stage[4]/stagesH[4]))
    ...
    def lstateStageDistribution(self, inputMat, columnState, columnStage):
        ...
        for i in lstates:
            ...
            vec_len = (length_awake/lstatePopulation, length_n1/lstatePopulation, length_n2/lstatePopulation, length_n3/lstatePopulation, length_rem/lstatePopulation)
    ...
    def MI_stimulusH_barPlot(self, MI_stage, stagesH, overAll, saveDir):
        ...
        ax1.bar(ind+width, MI_stage[1]/stagesH[1], bar_width, color=colors[4]['color'], edgecolor = "none", label="N1")
        ax1.bar(ind+2*width, MI_stage[2]/stagesH[2], bar_width, color=colors[4]['color'], edgecolor = "none", label="N2")
        ax1.bar(ind+3*width, MI_stage[3]/stagesH[3], bar_width, color=colors[4]['color'], edgecolor = "none", label="N3")
        ax1.bar(ind+4**width, MI_stage[4]/stagesH[4], bar_width, color=colors[1]['color'], edgecolor = "none", label="REM")
        ax1.bar(ind+7*width, overAll, bar_width, color='#40e0d0', edgecolor = "none", label="OverAll")
