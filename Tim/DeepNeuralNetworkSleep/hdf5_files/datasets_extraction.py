...
def myTraining_dataset_OS_RGS_CBD(file_path, n_states, n_repeat_states, rat_indices_OS, rat_indices_RGS,
                                  rat_indices_CBD):
    n_artefact, n_wake, n_nrem, n_TS, n_rem = n_states
    n_repeat_artefact, n_repeat_wake, n_repeat_nrem, n_repeat_TS, n_repeat_rem = n_repeat_states
    training_dataset = np.empty((0, 10))
    ...
    with h5py.File(file_path, 'r') as hdf:
        for i in rat_indices_OS:
            ...
            for group_name in hdf["OS_basic"][f"Rat{i}"]:
                ...
                array_scores = np.concatenate((array_scores, group['Mapped_scores'][:]), axis=0)
                array_features = np.concatenate((array_features, group['Features'][:]), axis=0)
            indices_artefact_total, indices_wake_total, indices_nrem_total, indices_TS_total, indices_rem_total, array_scores_total, array_features_total = update_indices(
                array_scores, array_features, indices_artefact_total, indices_wake_total, indices_nrem_total,
                indices_TS_total, indices_rem_total, array_scores_total, array_features_total)
    ...
    # Set the max valid index
    max_index = len(array_features_total) - 1

    # Adjust indices within bounds
    selected_indices_artefact = adjust_indices(indices_artefact_total, n_artefact, n_repeat_artefact, max_index)
    selected_indices_wake = adjust_indices(indices_wake_total, n_wake, n_repeat_wake, max_index)
    selected_indices_nrem = adjust_indices(indices_nrem_total, n_nrem, n_repeat_nrem, max_index)
    selected_indices_TS = adjust_indices(indices_TS_total, n_TS, n_repeat_TS, max_index)
    selected_indices_rem = adjust_indices(indices_rem_total, n_rem, n_repeat_rem, max_index)

    # Combine features and scores for each state
    training_dataset, manual_scoring = combine_features_scores(
        selected_indices_artefact, selected_indices_wake, selected_indices_nrem, selected_indices_TS,
        selected_indices_rem,
        array_features_total, array_scores_total, training_dataset, manual_scoring)
    ...
    training_dataset = np.array(training_dataset)
    manual_scoring = np.array(manual_scoring)
    return np.array(training_dataset), np.array(manual_scoring).astype(int)


def update_indices(array_scores, array_features, indices_artefact_total, indices_wake_total, indices_nrem_total,
                   indices_TS_total, indices_rem_total, array_scores_total, array_features_total):
    len_scores = len(array_scores_total)
    indices_artefact_total = np.concatenate((indices_artefact_total, np.where(array_scores == 0)[0] + len_scores),
                                            axis=0)
    indices_wake_total = np.concatenate((indices_wake_total, np.where(array_scores == 1)[0] + len_scores), axis=0)
    indices_nrem_total = np.concatenate((indices_nrem_total, np.where(array_scores == 3)[0] + len_scores), axis=0)
    indices_TS_total = np.concatenate((indices_TS_total, np.where(array_scores == 4)[0] + len_scores), axis=0)
    indices_rem_total = np.concatenate((indices_rem_total, np.where(array_scores == 5)[0] + len_scores), axis=0)
    array_scores_total = np.concatenate((array_scores_total, array_scores), axis=0)
    array_features_total = np.concatenate((array_features_total, array_features), axis=0)
    return indices_artefact_total, indices_wake_total, indices_nrem_total, indices_TS_total, indices_rem_total, array_scores_total, array_features_total


def adjust_indices(indices_total, n, n_repeat, max_index):
    selected_indices = random.sample(list(indices_total), n)
    selected_indices = np.ravel([[el] * n_repeat for el in selected_indices])
    return selected_indices[selected_indices <= max_index]


def combine_features_scores(selected_indices_artefact, selected_indices_wake, selected_indices_nrem,
                            selected_indices_TS, selected_indices_rem, array_features_total, array_scores_total,
                            training_dataset, manual_scoring):
    for indices in [selected_indices_artefact, selected_indices_wake, selected_indices_nrem, selected_indices_TS,
                    selected_indices_rem]:
        features = array_features_total[indices.astype(int)]
        scores = array_scores_total[indices.astype(int)]
        training_dataset = np.concatenate((training_dataset, features), axis=0)
        manual_scoring = np.concatenate((manual_scoring, scores), axis=0)
    return training_dataset, manual_scoring


def myDataset_OS_RGS_CBD(file_path, rat_indices_OS, rat_indices_RGS, rat_indices_CBD, rat_indices_PILOT,
                         rat_indices_TEST, states):
    dataset = np.empty((0, 10))
    manual_scoring = np.array([])
    ...
    with h5py.File(file_path, 'r') as hdf:
        for i in rat_indices_OS:
            ...
            array_scores_total = np.concatenate((array_scores_total, array_scores), axis=0)
            array_features_total = np.concatenate((array_features_total, array_features), axis=0)
            indices_artefact_total = np.concatenate(
                (indices_artefact_total, np.where(array_scores == 0)[0] + len_scores))
            indices_wake_total = np.concatenate((indices_wake_total, np.where(array_scores == 1)[0] + len_scores))
            indices_nrem_total = np.concatenate((indices_nrem_total, np.where(array_scores == 3)[0] + len_scores))
            indices_TS_total = np.concatenate((indices_TS_total, np.where(array_scores == 4)[0] + len_scores))
            indices_rem_total = np.concatenate((indices_rem_total, np.where(array_scores == 5)[0] + len_scores))
        ...
        for i in rat_indices_PILOT:
            array_scores = np.array([])
            array_features = np.empty((0, 10))
            group = hdf["pilot"][f"Rat{i}"]
            array_scores = np.concatenate((array_scores, group['Mapped_scores'][:]), axis=0)
            array_features = np.concatenate((array_features, group['Features'][:]), axis=0)
            len_scores = len(array_scores_total)
            array_scores_total = np.concatenate((array_scores_total, array_scores), axis=0)
        ...
        for i in rat_indices_TEST:
            array_scores = np.array([])
            array_features = np.empty((0, 10))
            group = hdf["test"][f"Rat{i}"]
            array_scores = np.concatenate((array_scores, group['Mapped_scores'][:]), axis=0)
            array_features = np.concatenate((array_features, group['Features'][:]), axis=0)
            len_scores = len(array_scores_total)
            array_scores_total = np.concatenate((array_scores_total, array_scores), axis=0)
    ...
    # Define max index to ensure within-bounds indices
    max_index = len(array_features_total) - 1

    # Adjust indices to keep within bounds
    indices_artefact_total = indices_artefact_total[indices_artefact_total <= max_index]
    indices_wake_total = indices_wake_total[indices_wake_total <= max_index]
    indices_nrem_total = indices_nrem_total[indices_nrem_total <= max_index]
    indices_TS_total = indices_TS_total[indices_TS_total <= max_index]
    indices_rem_total = indices_rem_total[indices_rem_total <= max_index]

    # Select data based on adjusted indices
    states = [el.lower() for el in states]
...
def myDataset_in_order_OS_RGS_CBD(file_path, rat_indexes_OS, rat_indexes_RGS, rat_indexes_CBD, states):
    """
    Extract a dataset from the OS_RGS_CBD.h5 file while keeping the epochs in a chronological order for each posttrial.
    The returned dataset only includes epochs with the requested states.
    """
    dataset = np.empty((0, 10))
    manual_scoring = np.array([])
    ...
    with h5py.File(file_path, 'r') as hdf:
        for dataset_name, rat_indices in [
            ("OS_basic", rat_indexes_OS),
            ("RGS14", rat_indexes_RGS),
            ("CBD", rat_indexes_CBD),
        ]:
            for i in rat_indices:
                for group_name in hdf[dataset_name][f"Rat{i}"]:
                    group = hdf[dataset_name][f"Rat{i}"][group_name]
                    array_scores = group['Mapped_scores'][:]
                    array_features = group['Features'][:]

                    # Check size alignment
                    assert len(array_scores) == array_features.shape[0], \
                        f"Mismatch between scores and features in {dataset_name} Rat{i} {group_name}"

                    # Filter rows matching the desired states
                    valid_mask = np.isin(array_scores, int_states)
                    filtered_scores = array_scores[valid_mask]
                    filtered_features = array_features[valid_mask]

                    # Protect against index overflow
                    if len(filtered_scores) != filtered_features.shape[0]:
                        min_len = min(len(filtered_scores), filtered_features.shape[0])
                        filtered_scores = filtered_scores[:min_len]
                        filtered_features = filtered_features[:min_len]

                    dataset = np.concatenate((dataset, filtered_features), axis=0)
                    manual_scoring = np.concatenate((manual_scoring, filtered_scores), axis=0)
...
def posttrial_OS_RGS_CBD(file_path, rat_dataset, rat_index, posttrial_name, states):
    ...
    with h5py.File(file_path, 'r') as hdf:
        if rat_dataset == "OS":
            ...
            array_scores = group['Mapped_scores'][:]
            array_features = group['Features'][:]
            # if len(array_scores) > len(array_features):
            # array_scores = array_scores[:len(array_features)]  # Trim extra element
            if int_states == [0, 1, 3, 4, 5]:
                dataset = np.concatenate((dataset, array_features), axis=0)
                manual_scoring = np.concatenate((manual_scoring, array_scores), axis=0)
            else:
                for i, el in enumerate(array_scores):
                    if el in int_states:
                        # if i < len(array_features):
                        # dataset = np.concatenate((dataset, [array_features[i]]), axis=0)
                        dataset = np.concatenate((dataset, [array_features[i]]), axis=0)
                        manual_scoring = np.append(manual_scoring, el)

        elif rat_dataset == "pilot":
            group = hdf["pilot"][f"Rat{rat_index}"]
            array_scores = group['Mapped_scores'][:]
            array_features = group['Features'][:]
            # if len(array_scores) > len(array_features):
            # array_scores = array_scores[:len(array_features)]  # Trim extra element
        ...
        elif rat_dataset == "test":
            group = hdf["test"][f"Rat{rat_index}"]
            array_scores = group['Mapped_scores'][:]
            array_features = group['Features'][:]
            # if len(array_scores) > len(array_features):
            # array_scores = array_scores[:len(array_features)]  # Trim extra element
...
if __name__ == "__main__":
    print(dataset_in_order_OS_RGS_CBD(
        "C:/Users/timmi/Documents/merged/merged.h5",
        [1], [], [],
