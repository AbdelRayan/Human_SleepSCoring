...
def rem_feature(cross_cor_val, auto_slope):
    rem_features = []
    for count, slope in enumerate(auto_slope):
        rem_features.append((1/slope)*np.sign(cross_cor_val[count]))
    return rem_features
...
if __name__ == "__main__":
    ...
    cross_cor_values = cross_correlation(EOG1, EOG2, epoch_length, fs, 0)
    auto_cor_slope = auto_correlation(EOG1, epoch_length, fs)
    rem_features = rem_feature(cross_cor_values, auto_cor_slope)

    print(len(auto_cor_slope))

    fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(28, 10))
    eog_epochs = np.arange(len(auto_cor_slope))
    hypno_epochs = np.arange(len(states))
    ax1.plot(eog_epochs, rem_features)
    ax2.plot(hypno_epochs,states)

    ax2.set_yticks([0, 1, 2, 3, 4])
    ax2.invert_yaxis()
    fig.savefig('rem_feature.svg', format = "svg")
    plt.show()