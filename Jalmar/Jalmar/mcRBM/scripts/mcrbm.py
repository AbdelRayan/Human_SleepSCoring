...
class mcRBM:
    ...
    def train(self):
        ...
        w_mean = 0.05 * prng.randn(num_vis, self.num_hid_mean).astype(xp.float32)
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch in range(num_batches):
                ...
                if use_hidden_dropout:
                    ...
                    cov_drop = np.random.binomial(1, keep_cov, size=(self.num_hid_cov, self.batch_size)).astype(np.float32) / keep_cov
                    mean_drop = np.random.binomial(1, keep_mean, size=(self.num_hid_mean, self.batch_size)).astype(np.float32) / keep_mean
                else:
                    cov_drop = 1.0
                    mean_drop = 1.0
                ...
                # HMC sampling
                negdata = xp.random.randn(num_vis, self.batch_size).astype(xp.float32)
            ...
            if epoch % 100 == 0 or epoch == self.num_epochs - 1:
                ...
                ax.cla()
                ax.plot(meanEnergy[:epoch+1], label='Mean Energy', marker='o')
                ax.plot(maxEnergy[:epoch+1], label='Max Energy', marker='s')
                ax.plot(minEnergy[:epoch+1], label='Min Energy', marker='^')
                ax.legend()
                ax.set_xlabel('Epoch')
